...
Eine Übersicht über die hier zur Erklärung verwendeten Interfaces und Klassen ist in \autoref{pic:MoveInterfaces} gezeigt.
...
Dame, Läufer und Turm sind in ihrem Handlungsspielraum recht ähnlich. Ein Läufer kann nur schräg, ein Turm nur geradeaus ziehen. Eine Dame beherrscht beide dieser Zugarten. In unserem Code haben wir sowohl ein Interface für gerade Züge, als auch schräge Züge. Die oben genannten drei Figuren implementieren jeweils die Methoden für IsMoveable und MoveIsValid. 



Die Implementierung der Methode \texttt{StraightMoveIsValid} des Turms findet sich so $1:1$ bei der Dame wieder. Die Methode der Dame kopiert zudem auch die Methoden zur schrägen Zugvalidierung welche auch im Läufer zu finden sind. Dadurch sind die Methoden für schräges und gerades Laufen doppelt implementiert ohne Unterschied.

\subsubsection{Positivbeispiel}
Die unter \autoref{sec:replTemp} genutzte Methode \texttt{GetSquareDistance} wird von fast jeder Figur verwendet. Anstatt diese jeweils innerhalb einer Figurenklasse wie \texttt{Queen} zu platzieren, ist die Methode in \texttt{Piece} implementiert. Da alle Figuren von \texttt{Piece} erben, können sie auf die Methode zugreifen, welche jedoch nur ein einziges Mal im Code ausprogrammiert ist. Dadurch ist hierbei, wenn auch nur mit einer sehr simplem Methode \textit{DRY} umgesetzt.
